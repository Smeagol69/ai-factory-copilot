...
test("capture dimensions use complete native bounds and verify archive and disk results", () => {
  ...
  assert.match(exporter, /actor_origin_envelope_fallback_native_bounds_unknown/);
  assert.match(exporter, /ExpectedCaptureDimensions = CaptureDimensions/);
  for (const axis of ["X", "Y", "Z"]) {
    assert.match(exporter, new RegExp(`DimensionJson->SetNumberField\\(TEXT\\("${axis.toLowerCase()}"\\), ExpectedCaptureDimensions\\.${axis}\\)`));
  }
