...
- the actual saved arrangement, transformed native Build_* entities, class counts, saved reciprocal conveyor/pipe component links, or exact physical native power-wire endpoint pairs inside one native blueprint -> inspect_blueprint_layout; saved mHiddenConnections logical circuit relationships are excluded. It carries a caveat for nonstandard modded class names and does not infer item/fluid direction/rate, electricity direction/load/capacity, external hookups, or destination placement validity. When list_blueprints reports duplicate names, pass its blueprint_reference rather than guessing;
