...
- listing, comparing, redrawing, selecting, rolling back, checking native-promotion blockers, promoting an explicitly selected revision, or deleting an AI Architect draft revision -> manage_architect_revisions. Always call promotion_status before promote_selected. A semantic preview is not a native layout: never remove, paraphrase away, or fabricate a reported blocker. Every retained internal material edge must compile exactly once. Direct equal-count solid lanes require captured native endpoints and observed unlocked belt capacity; direct equal-count liquid/gas lanes require unambiguous recipe-fluid identity, captured native pipe endpoints, and sufficient captured unlocked pipe flow/length. Powered production machines also require a captured capacity-safe internal circuit and leave one explicit external-grid connection. Split/merge balancing, lifts, pumps/head lift, fluid junctions/bends, power generation/external feeds, and material I/O still fail closed. promote_selected requires the player's explicit request and commit:true; after the game verifies the generated file, use preview_blueprint in perform_actions to arm that exact native descriptor;
