...
export const SOLVER_TOOLS = [
...

  {
    ...
    description:
      "Manages save/session-scoped AI Architect revisions. Use list/get to inspect immutable options, compare for exact geometry/production/topology/style/blocker deltas, preview to redraw one stored option, select or rollback to choose it, promotion_status to recompile the exact selected revision and list every missing native-placement or topology adapter, promote_selected only after the player explicitly asks to write the proven layout through the existing native Designer/serializer, and delete_draft to remove an unselected leaf draft. Promotion never guesses or drops a semantic element or material edge: it emits one standalone generate_native_blueprint action only when the revision, current unlock fingerprint, selected recipes, descriptor dimensions, every element compiler, and the existing generated-Blueprint contract all pass. Direct equal-count solid-material dependencies with exact rates, captured endpoints, and observed unlocked belt capacity compile through generated-Blueprint v2. Powered production machines also require captured circuit connector identity/capacity/position, compatible circuit types, and an unlocked wire with captured maximum length; they receive either a capacity-safe daisy chain or captured compatible pole trunk with one external-grid link reserved. Split/merge, lift, fluid, generation/external feeds, and external-material-I/O requirements remain exact blockers. Native generation still does not place a factory; after verified game readback, use preview_blueprint to arm the saved descriptor in the player's Build Gun.",
    parameters: {
      type: "object",
      properties: {
        operation: {
