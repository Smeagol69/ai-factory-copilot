...
- Existing vanilla nodes can be inspected and can use the game's reversible
  resource-class override only when they are ordinary, unoccupied `Node`
  actors; deposits, geysers, fracking actors, and transient Blueprint Resource
  Anchor nodes are never changed through that generic path. Vanilla nodes are
  never moved, deleted, or adopted.
- `AI Factory Copilot Creative Node` now exists in source as a concrete
  ...
  configuration. It remains static after placement; any future
  spawn-replacement move or unoccupied-only removal needs its own verified
  design and live proof.
