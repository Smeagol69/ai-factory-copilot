...
| 2026-08-26 | Codex | `codex/creative-node-integration` | Integrate and harden the existing native creative resource-node experiment as the safe starting point for a true Build-Gun-driven world editor. Scope: rebase the mod-owned saveable node/hologram/RCO path, preserve Blueprint Resource Anchor behavior, reject all nonordinary and transient node classes from generic editing, pin source contracts, compile against the official 502094 headers, and document the live test matrix. It will not implement Claude's raw `SpawnNodeLike`, mutate vanilla resource nodes without an explicit supported path, remove existing Blueprint/Anchor features, change belts/pipes, or claim a live node editor before it is actually tested. | claimed; no implementation before this notice is pushed |
