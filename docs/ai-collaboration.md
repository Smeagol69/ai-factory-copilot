...
|---|---|---|---|---|
| 2026-08-09 | Codex | `codex/release-hardening` | Current-unlock build constraints: narrow bridge rejection for locked `place_building` recipes in `companion/lib/actions.mjs`; deterministic unlock fingerprint and replan/optimization provenance in `companion/lib/megabase.mjs` / tool output; focused tests and planning docs. C++ remains unchanged because its authoritative `AFGRecipeManager` preflight already rejects locked build and production recipes. | in progress |
