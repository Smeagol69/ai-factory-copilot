...
|---|---|---|---|---|
| 2026-08-30 | Codex | `codex/creative-node-delete` | Add an explicit safe removal workflow for spawned Creative Resource Nodes now that ordinary-node Miner compatibility is live-verified. Scope removal to an aimed, unoccupied, mod-owned ordinary creative node or creative geyser; require the existing write/admin authority gates plus a short-lived second confirmation; execute only on the server and report exact readback. Never delete or retarget vanilla map nodes, deposits, Blueprint Anchor runtime nodes, occupied nodes, arbitrary mod templates, or a node whose identity changed between confirmation and commit. Preserve Node Spawner/Editor, Miner compatibility, resource discovery, generated Blueprints, and all existing chat/AI actions. Expected files: creative node chat/edit authority path, focused UI if the native Node Editor can expose the command without duplicating validation, source-contract tests, editor docs/changelog, and this append-only handoff. | claimed; inspecting the existing Node Editor/RCO authority path and exact destruction lifecycle before implementation |
