...
|---|---|---|---|---|
| 2026-08-23 | Codex | `codex/blueprint-preview-library` | Fix the live-observed native Blueprint preview library seam only: a companion-resolved in-root `.sbp` must be refreshed into Satisfactory's authoritative Blueprint library before the existing client Build Gun handoff, or fail with an exact reason. Scope is `AIFactoryActions` plus focused source-contract tests/docs/exact-header validation. Preserve Blueprint export, selection overlay, native RCO, and all no-placement/no-charge preview guarantees. | claimed; no implementation before this notice is pushed |
