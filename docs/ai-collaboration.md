...
|---|---|---|---|---|
| 2026-08-24 | Codex | `codex/blueprint-connection-topology` | Companion-only, read-only native Blueprint conveyor/pipe topology decoder: resolve serialized component references to their owning native entities, validate reciprocal belt/pipe links, and surface only bounded, evidence-backed topology in `inspect_blueprint_layout`. Scope is `companion/lib/blueprints.mjs`, its existing solver/router/tool contract, focused tests, and docs. Power/wire topology, connector direction, placement validity, terrain, costs, and all world writes remain explicitly out of scope. | claimed; no implementation before this notice is pushed |
