...
|---|---|---|---|---|
| 2026-08-23 | Codex | `codex/blueprint-designer-miners` | **Correction to the handoff above:** the existing 18,682,703-byte beta.2 archive was packaged before `d19d8a0`'s fail-closed miner-restoration code. It proved the hook-removal cook path only; it does not contain the final restoration hardening. | **Do not deploy or publish that archive.** Source is now at `ddb8996` (after `d19d8a0`), with 809/809 tests and source builds passing. Close Satisfactory PID 13232, rebuild from this branch/integrated commit, then run the native Designer place/save/reload/dismantle/host-client matrix before a unique prerelease is created. |
| 2026-08-23 | Codex | `codex/blueprint-designer-miners` | **Handoff:** native Blueprint Designer Miner Mk.1–Mk.3 support is implemented through a narrow Resource Anchor root. It owns a transient real `AFGResourceNode`; the normal Build Gun places both anchor and Miner, and exact native extractor-pointer bindings are persisted/recreated without spatial guesses. The implementation removes only vanilla Mk.1–Mk.3 from the Designer blacklist; fluids, oil, fracking, portable, and modded extractors stay unchanged. The unsafe `SetExtractableResource` SML trampoline was removed after a package failure; engine-native `SetIsOccupied` now records claims and restore refuses stale or occupied mappings. | **Source complete and pushed:** `dc5c59a`, `088ab37`, `d19d8a0`. 809/809 companion tests, exact headers, Shipping + Editor builds, and UAT package pass. Archive: 18,682,703 bytes, SHA-256 `BF36DCFAF18C72F2F302AE207387F80A264A9AAEEDA982605BD9ABF99F704BEC`. **Do not call live-proven:** Satisfactory PID 13232 is open, so deployment and Designer place/save/reload/dismantle/host-client proof remain the next gate. |
