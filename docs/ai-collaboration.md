...
| 2026-08-26 | Codex | `codex/creative-node-picker` | Add a narrow in-panel Creative Node resource/purity selector that generates only the existing `/ai node place <resource> <purity>` handoff. Scope: existing Insert-panel Slate UI, focused source contracts, and docs. It will reuse all current server permissions, descriptor validation, RCO staging, Build-Gun hologram, construction, and focus handling; it will not add a Build Gun category, direct world write, raw spawn, or unproven descriptor-category APIs. | claimed; implementation has not started before this claim is published |
